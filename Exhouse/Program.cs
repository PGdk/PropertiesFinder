    ...
    class Program
    {
        static void Main()
        {
            ...
            foreach (var webSiteIntegrationType in webSiteIntegrationsTypes)
            {
                ...
                foreach (var oldDumpDetails in oldDumpsDetails)
                {
                    ...
                    var closedEntries = oldDump.Entries
                        .Where(e => e.OfferDetails.IsStillValid == true)
                        .Except(newDump.Entries, webSiteIngegration.EntriesComparer);

                    foreach (var closedEntry in closedEntries)
                    {
        ...
        private static IEnumerable<Type> GetTypesThatImplementsInterface(Type interfaceType)
        {
            ...
            return assemblies
                ...
                .Where(type => interfaceType.IsAssignableFrom(type) && type.GetTypeInfo().IsClass);
        }
    }
}
