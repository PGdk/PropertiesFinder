...
namespace Bazos
{
    public class BazosIntegration : IWebSiteIntegration
    {
        ...
        public static List<Entry> GenerateDump(int id)
        {
            List<string> pages = new List<string>();
            HtmlWeb web = new HtmlWeb();
            int pageNumber = id * 20;
            HtmlDocument doc = web.Load("https://nieruchomosci.bazos.pl/" + pageNumber + "/");
        ...
        private static void GetAllPages(List<string> pages, HtmlDocument doc, string url)
        {
            ...
            InfoExtracter infoExtracter = new InfoExtracter(new PolishStringParser(), new DocumentChecker());
        ...
        private static void CreateDump(List<string> pages, HtmlWeb web, HtmlDocument doc, List<Entry> dumpEntries)
        {
            foreach (var page in pages)
            {
                ...
                doc = web.Load(page);
                InfoExtracter infoExtracter = new InfoExtracter(new PolishStringParser(), new DocumentChecker());
                if(!infoExtracter.ExtractInfoFromPropertyPage(info, doc))
                {
                    continue;
                }
                decimal ppm;
