...
namespace Aplication
{
    public class DomiportaIntegration : IWebSiteIntegration
    {
        ...
        public static List<Entry> GetEntriesPageID(int id)
        {
            var web = new HtmlAgilityPack.HtmlWeb();
            List<string> LinksToAds = new List<string>();

            //for (int i = 1; i <= 1; i++) // i = obsluga numeru strony ! TODO
            //{
            //    if (i == id)
            //    {
            //        var doc = web.Load("https://www.domiporta.pl/mieszkanie/sprzedam?Rodzaj=Bezposrednie&PageNumber=" + pageNumber);

            //        AddLinksToList(doc, LinksToAds);
            //    }
            //}

            var doc = web.Load("https://www.domiporta.pl/mieszkanie/sprzedam?Rodzaj=Bezposrednie&PageNumber=" + id);
            AddLinksToList(doc, LinksToAds);
            ...
            for (int i = 0; i < LinksToAds.Count; i++)
            {
                var docE = web.Load("https://www.domiporta.pl" + LinksToAds[i]);
                EntriesTmp.Add(CreateEntry(docE, LinksToAds[i]));
            }
